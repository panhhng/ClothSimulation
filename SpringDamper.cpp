...
SpringDamper::SpringDamper(Particle *p_1, Particle *p_2, float l0, float ks, float kd)
    : p1(p_1), p2(p_2), springConst(ks), dampFactor(kd), restLen(l0), broken(false)
{
}

bool SpringDamper::isBroken() const {
    return broken; 
}
...
void SpringDamper::computeForce()
{
    ...
    float tearLimit = restLen * 2.0;
    if (currLen > tearLimit)
    {
        broken = true;
        return;
    }
